...
def main (app):
	...
	resp, content = h.request("http://crowd.skytruth.org/app/%s/tasks/export?type=task&format=json" % app, "GET")
		
	
	tasks = dict((task['id'], task)
	             for task in json.loads(content))
	
	resp, content = h.request("http://crowd.skytruth.org/app/%s/tasks/export?type=task_run&format=json" % app, "GET")
